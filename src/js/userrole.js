...
Component.entryPoint = function(NS){
	...
	RoleRowWidget.prototype = {
		...
		onClick: function(el){
			if (!L.isNull(this.helpWidget)){
				if (this.helpWidget.onClick(el)){ return true; }
			}
			
			var tp = this._TId['urrow'];
			switch(el.id){
			case tp['binfo']: this.showHelp(); return true;
			}
			return false;
		},
		...
		updateHelp: function(){
			if (L.isNull(this.helpWidget)){ return; }
	...
	RoleListWidget.prototype = {
		...
		onClick: function(el){
			...
			case tp['bok']: this.applyEdChanges(); return true;
			case tp['bcancel']: this.cancelEdChanges(); return true;
			}
			return false;
		},
		...
		showEditor: function(){
			if (!L.isNull(this.usersWidget)){ return; }
		...
		applyEdChanges: function(){
			...
			for (var i=0;i<ws.length;i++){
				...
				for (var ii=0;ii<uIds.length;ii++){
					if (ws[i].role.userid == uIds[ii]){ find = true; }
				}
				if (!find){
					ws[i].destroy();
				}else{
